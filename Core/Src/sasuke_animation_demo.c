...
typedef struct {
  SasukeMoveState state;
  const char *label;
} DemoStateStep;

static const DemoStateStep s_sequence[] = {
    {SASUKE_MOVE_IDLE, "IDLE"},
    {SASUKE_MOVE_RUN, "RUN"},
    {SASUKE_MOVE_JUMP_STARTUP, "JUMP START"},
    {SASUKE_MOVE_JUMP_AIR, "JUMP AIR"},
    {SASUKE_MOVE_FALL, "FALL"},
    {SASUKE_MOVE_JUMP_LAND, "LAND"},
    {SASUKE_MOVE_ATTACK1, "ATTACK 1"},
    {SASUKE_MOVE_ATTACK2, "ATTACK 2"},
    {SASUKE_MOVE_ATTACK3, "ATTACK 3"},
    {SASUKE_MOVE_BLOCK, "BLOCK"},
    {SASUKE_MOVE_SKILL, "CHIDORI"},
    {SASUKE_MOVE_HIT, "HIT"},
    {SASUKE_MOVE_DEAD, "DEAD"}};
...
static const SasukeMoveFrame *s_previousFrame;
...
void SasukeAnimationDemo_Init(void) {
  LCD_Port_Init();
  Demo_DrawBackground();
...
void SasukeAnimationDemo_Update(void) {
...
static void Demo_DrawCurrentFrame(void) {
  const SasukeMoveAnimation *animation =
      &sasuke_move_animations[s_sequence[s_sequenceIndex].state];
  const SasukeMoveFrame *frame = &animation->frames[s_frameIndex];
...
static void Demo_DrawFrameTransition(uint8_t nextFrameIndex) {
  const SasukeMoveAnimation *animation =
      &sasuke_move_animations[s_sequence[s_sequenceIndex].state];
  const SasukeMoveFrame *newFrame = &animation->frames[nextFrameIndex];
...
static int16_t Demo_GetFrameOffsetY(SasukeMoveState state, uint8_t frameIndex) {
  if (state == SASUKE_MOVE_JUMP_AIR) {
    return -30;
  }
  if (state == SASUKE_MOVE_FALL) {
    return -15;
  }
  return 0;
}
...
static void Demo_DrawStateLabel(const char *label) {
  ...
  ILI9341_DrawText("SASUKE:", FONT2, 12U, 9U, RGB565_ACCENT_CYAN,
                   RGB565_LABEL_BG);
