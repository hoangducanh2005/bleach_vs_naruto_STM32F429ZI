...
/**
 ******************************************************************************
 * @file           : naruto_full_nine_tails_animation_demo.c
 * @brief          : Naruto Full Nine Tails moveset animation demo for the STM32 LCD.
 ******************************************************************************
 */
/* USER CODE END Header */

#include "naruto_full_nine_tails_animation_demo.h"
...
typedef struct {
  NarutoFullNineTailsMoveState state;
  const char *label;
} DemoStateStep;

static const DemoStateStep s_sequence[] = {
    {NARUTO_FULL_NINE_TAILS_MOVE_IDLE, "IDLE"},
    {NARUTO_FULL_NINE_TAILS_MOVE_RUN, "RUN"},
    {NARUTO_FULL_NINE_TAILS_MOVE_SLIDE, "SLIDE"},
    {NARUTO_FULL_NINE_TAILS_MOVE_JUMP, "JUMP"},
    {NARUTO_FULL_NINE_TAILS_MOVE_BLOCK, "BLOCK"},
    {NARUTO_FULL_NINE_TAILS_MOVE_HIT, "HIT"},
    {NARUTO_FULL_NINE_TAILS_MOVE_ATTACK1, "ATTACK 1"},
    {NARUTO_FULL_NINE_TAILS_MOVE_ATTACK2, "ATTACK 2"},
    {NARUTO_FULL_NINE_TAILS_MOVE_ATTACK3, "ATTACK 3"},
    {NARUTO_FULL_NINE_TAILS_MOVE_ATTACK4, "ATTACK 4"},
    {NARUTO_FULL_NINE_TAILS_MOVE_SKILL1, "SKILL 1"},
    {NARUTO_FULL_NINE_TAILS_MOVE_SKILL2, "SKILL 2"},
    {NARUTO_FULL_NINE_TAILS_MOVE_SKILL3, "SKILL 3"},
    {NARUTO_FULL_NINE_TAILS_MOVE_DEAD, "DEAD"}};

static const int16_t s_jumpOffsetsY[] = {0, -10, -22, -34, -34, -22, -10, 0};
...
static int16_t s_statePivotX;
static const NarutoFullNineTailsMoveFrame *s_previousFrame;
...
static int16_t Demo_GetFrameX(const NarutoFullNineTailsMoveFrame *frame);
static int16_t Demo_GetFrameOffsetY(NarutoFullNineTailsMoveState state, uint8_t frameIndex);
...
void NarutoFullNineTailsAnimationDemo_Init(void) {
  LCD_Port_Init();
  Demo_DrawBackground();
...
void NarutoFullNineTailsAnimationDemo_Update(void) {
  ...
  const NarutoFullNineTailsMoveAnimation *animation =
      &naruto_full_nine_tails_move_animations[s_sequence[s_sequenceIndex].state];
...
static void Demo_DrawCurrentFrame(void) {
  const NarutoFullNineTailsMoveAnimation *animation =
      &naruto_full_nine_tails_move_animations[s_sequence[s_sequenceIndex].state];
  const NarutoFullNineTailsMoveFrame *frame = &animation->frames[s_frameIndex];
  int16_t x = Demo_GetFrameX(frame);
...
static void Demo_DrawFrameTransition(uint8_t nextFrameIndex) {
  const NarutoFullNineTailsMoveAnimation *animation =
      &naruto_full_nine_tails_move_animations[s_sequence[s_sequenceIndex].state];
  const NarutoFullNineTailsMoveFrame *newFrame = &animation->frames[nextFrameIndex];
  int16_t newX = Demo_GetFrameX(newFrame);
  int16_t newY =
...
static int16_t Demo_GetFrameOffsetY(NarutoFullNineTailsMoveState state, uint8_t frameIndex) {
  if ((state != NARUTO_FULL_NINE_TAILS_MOVE_JUMP) ||
...
static int16_t Demo_GetFrameX(const NarutoFullNineTailsMoveFrame *frame) {
  int16_t x = (int16_t)(DEMO_ANCHOR_X - s_statePivotX);
  int16_t deltaPivot = (int16_t)(frame->pivotX - s_statePivotX);

  if (deltaPivot > 18) {
    x = (int16_t)(x - 18);
  } else if (deltaPivot < -18) {
    x = (int16_t)(x + 18);
  }
...
static void Demo_DrawStateLabel(const char *label) {
  ...
  ILI9341_DrawText("NINE:", FONT2, 12U, 9U, RGB565_ACCENT_CYAN,
                   RGB565_LABEL_BG);
  ILI9341_DrawText(label, FONT2, 58U, 9U, RGB565_WHITE, RGB565_LABEL_BG);
}
