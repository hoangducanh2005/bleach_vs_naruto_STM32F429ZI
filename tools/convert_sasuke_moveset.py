...
CONFIG = ROOT / "assets" / "raw" / "sasuke_frames" / "sasuke_moveset.json"
OUT_H = ROOT / "Core" / "Inc" / "sasuke_moveset.h"
OUT_C = ROOT / "Core" / "Src" / "sasuke_moveset.c"
...
def state_macro(name):
    return f"SASUKE_MOVE_{name.upper()}"


def state_symbol(name):
    return f"sasuke_move_{name}"


def load_frame(source_dir, frame_number):
    path = source_dir / f"sasuke_{frame_number:03d}.png"
...
"""
    ...
    lines.append(f"  SASUKE_MOVE_STATE_COUNT = {len(states)}U\n")
    lines.append(
        """} SasukeMoveState;
...
"""
    ...
    for state in states:
        ...
        lines.append(f"static const SasukeMoveFrame {symbol}_frames[] = {{\n")
        for idx, frame in enumerate(state["frames"]):
    ...
    lines.append("const SasukeMoveAnimation sasuke_move_animations[SASUKE_MOVE_STATE_COUNT] = {\n")
    ...
    for idx, state in enumerate(states):
        ...
        # Caster effect and projectile placeholders
        # Add actual logic if Sasuke gets specific effects mapped in moveset JSON
