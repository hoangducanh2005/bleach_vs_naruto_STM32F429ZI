...
def state_macro(name):
    return f"ICHIGO_MOVE_{name.upper()}"


def state_symbol(name):
    return f"ichigo_move_{name}"


def load_frame(source_dir, frame_number):
    path = source_dir / f"ichigo_{frame_number:03d}.png"
...
"""
    ...
    lines.append(f"  ICHIGO_MOVE_STATE_COUNT = {len(states)}U\n")
    lines.append(
        """} IchigoMoveState;
...
"""
    ...
    for state in states:
        ...
        lines.append(f"static const IchigoMoveFrame {symbol}_frames[] = {{\n")
        for idx, frame in enumerate(state["frames"]):
    ...
    lines.append("const IchigoMoveAnimation ichigo_move_animations[ICHIGO_MOVE_STATE_COUNT] = {\n")
    ...
    for idx, state in enumerate(states):
        ...
        if state["reuse_caster_effect"] == "ichigo_getsuga_frames":
            caster_effect_frames = "ichigo_getsuga_frames"
            caster_effect_count = "ICHIGO_GETSUGA_FRAME_COUNT"
            caster_effect_width = "ICHIGO_GETSUGA_WIDTH"
            caster_effect_height = "ICHIGO_GETSUGA_HEIGHT"

        if state["reuse_projectile"] == "getsuga_projectile_frames":
            projectile_frames = "getsuga_projectile_frames"
            projectile_count = "GETSUGA_PROJECTILE_FRAME_COUNT"
            projectile_width = "GETSUGA_PROJECTILE_WIDTH"
            projectile_height = "GETSUGA_PROJECTILE_HEIGHT"
